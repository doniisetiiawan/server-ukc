...
module.exports = (mongoose, mongooseDelete) => {
  ...
  Schema.statics.authenticate = function (params = {}) {
    return new Promise((resolve, reject) => {
      try {
        const emptries = ['email', 'password'].filter((key) => [undefined].includes(params[key]));
    ...
    return new Promise((resolve, reject) => {
      try {
        const permitParams = permitFields.reduce(
          (obj, key) => (params[key] === undefined
            ? obj
            : { ...obj, [key]: params[key] }),
          { ...extra },
        );

        const emptries = [...requiredFields, 'confirmPassword'].filter(
          (key) => [undefined].includes(params[key]),
        );
        ...
        this.findOne({ _id })
          .then((user) => {
            ...
            const permitParams = Object.entries(params || {}).reduce(
              (obj, [key, value]) => (permitFields.includes(key)
                ? { ...obj, [key]: value }
                : obj),
              {},
            );
